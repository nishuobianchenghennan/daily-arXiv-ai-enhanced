...
TL;DR: 本文研究了远程能带混合如何影响扭曲MoTe₂中分数陈绝缘体的稳定性，并解释了为何在填充数ν=1/3和ν=2/3处观察到不对称性。


<details>
  <summary>Details</summary>
Motivation: 实验观察到在扭曲的二碲化钼（MoTe₂）中，填充数ν=1/3和ν=2/3的分数陈绝缘体稳定性存在不对称性，本研究旨在探究其根源。

Method: 通过一系列近似莫尔材料连续谱描述的模型，研究远程能带混合对分数陈绝缘体稳定性的影响，并分析分数陈绝缘体、电子维格纳晶体和空穴维格纳晶体之间的竞争。

Result: 在没有能带混合时，ν=1/3处的主要竞争相是电子晶体，而ν=2/3处是空穴晶体。远程能带混合会显著降低电子晶体的能量，但对空穴晶体影响很小，因此它对ν=1/3处分数陈绝缘体的破坏作用远强于ν=2/3处。

Conclusion: 远程能带混合是导致扭曲MoTe₂中ν=1/3和ν=2/3不对称性的原因。该机制通过显著稳定在ν=1/3处的竞争相（电子维格纳晶体），从而破坏了该填充下的分数陈绝缘体。这一机制也解释了在ν>1/2时观察到的重入整数量子反常霍尔效应。
...
### [10] [Bose metal near pair-density-wave order in a spin-orbit-coupled Kondo lattice](https://arxiv.org/abs/2604.18451)
...
### [11] [Enhanced Anomalous Nernst Effect in the Ferromagnetic Kondo Lattice CeCo2As2](https://arxiv.org/abs/2604.17987)
...
### [12] [Fingerprints of preformed pairs in two-electron angle-resolved photoemission spectroscopy](https://arxiv.org/abs/2604.18560)
...
### [13] [Magnetic-fluctuation-driven suppression of spin-orbit hybridization in the surface ferromagnet GdAg$_2$/Ag(111)](https://arxiv.org/abs/2604.18009)
...
### [14] [Magnetotransport and Phase competition in three-dimensional Hubbard-Holstein model at half-filling](https://arxiv.org/abs/2604.18219)
...
### [15] [Photoinduced orbital polarization and Jahn-Teller effect in RNiO$_3$](https://arxiv.org/abs/2604.18524)
...
TL;DR: 本文研究了如何利用变分量子算法在量子计算机上计算材料的声子谱和热力学性质，并以硅和石墨烯为例，证明了该方法可以准确复现经典结果，为评估近期量子算法提供了一个新的物理基准。


<details>
  <summary>Details</summary>
Motivation: 变分量子算法在解决电子结构问题上展现了潜力，但其在其他领域的应用仍有待探索。因此，本文旨在研究如何将这类算法应用于计算材料的晶格振动（声子）和相关的热力学性质。

Method: 研究人员使用了变分量子本征求解器（VQE）和变分量子压缩（VQD）算法。他们首先通过密度泛函理论（DFT）获得声子哈密顿量，然后将其映射为量子比特算符，用以计算晶体硅和石墨烯的完整声子谱。研究还结合了错误缓解策略来提高在真实量子硬件上的计算精度。

Result: 该方法成功计算出了硅和石墨烯的声子色散曲线，并与经典对角化方法的计算结果进行了直接比较。基于量子计算得到的声子谱，进一步评估了振动熵、定容比热和热膨胀系数等热力学性质，成功再现了预期的低温量子行为和高温经典极限（杜隆-珀蒂定律）。

Conclusion: 尽管在计算效率上，经典方法目前仍占优，但本研究成功地将基于声子的热力学计算确立为一个严格且物理意义清晰的基准（benchmark），可用于评估近期量子设备上的变分量子算法性能。
...
### [34] [pyzentropy: A Python package implementing recursive entropy for first-principles thermodynamics](https://arxiv.org/abs/2604.17665)
...
TL;DR: 本文研究了用于在 MoS₂ 晶体管上沉积电介质的“种子层”（特别是钽）如何影响其性能。研究发现，种子层的厚度和沉积方法至关重要：它们会产生缺陷，但也能控制电荷转移。在贫氧环境下沉积的超薄钽层可获得最佳性能，这凸显了种子层工程是改进二维半导体器件的关键。


<details>
  <summary>Details</summary>
Motivation: 将二维半导体（如 MoS₂）与电介质材料集成是未来逻辑技术的核心挑战。通常会引入种子层来促进电介质的成核和附着，但本文旨在深入探究种子层如何关键性地影响电荷转移掺杂，并进而影响晶体管的性能。

Method: 研究人员制作了背栅单层 MoS₂ 晶体管，其顶面用 Ta-seed/HfOₓ 电介质叠层进行钝化。他们通过拉曼光谱、光致发光光谱和X射线光电子能谱等多种光谱学方法，对器件进行了电学和物理表征，并系统地改变了Ta种子层的厚度和沉积条件。

Result: 结果显示，晶体管的阈值电压和导通电流随Ta种子层的厚度和沉积条件而显著变化。种子层不仅在 MoS₂ 沟道中引入了无序，还改变了界面电荷环境。研究发现，在贫氧条件下沉积的超薄0.2纳米Ta种子层可限制沉积引起的损伤，同时促进电荷转移，从而获得了更好的性能。

Conclusion: 该研究将种子层工程确定为控制 MoS₂ 器件中无序和界面掺杂的关键策略。同时，它证明了在制造过程中使用多模式光谱学是进行工艺开发和监控的实用方法。
...
### [39] [Multipolar Piezoelectricity and Anisotropic Surface Transport in Alterelectrics](https://arxiv.org/abs/2604.18324)
...
### [40] [A unified framework for grain boundary distributions in textured materials](https://arxiv.org/abs/2604.17997)
...
### [41] [Materials Informatics Across the Length Scales](https://arxiv.org/abs/2604.18086)
...
TL;DR: 这篇论文综述了数据驱动方法（材料信息学）在材料科学中跨尺度（纳米、介观、微观到连续介观）的应用，讨论了其现有能力、挑战和未来方向。


<details>
  <summary>Details</summary>
Motivation: 尽管材料信息学发展迅速，但其方法在不同尺度上的可靠性和可移植性差异很大。因此，本文旨在全面审视这些数据驱动方法在各个尺度上的应用现状，明确当前的可靠技术，并找出阻碍其更广泛整合的关键障碍。

Method: 本文采用综述和分析的方法，系统性地调研了纳米尺度、介观尺度和微观到连续介观尺度的数据驱动方法。具体讨论了机器学习原子间势、介观代理模型和算子学习模型，以及基于学习的实验微观结构分析，并探讨了数据标准和自主实验室等新兴工具。

Result: 论文分析得出，当前材料信息学在不同尺度上的应用能力和成熟度不一。文章重点指出了在数据质量、不确定性、可解释性和跨尺度一致性方面存在的既有能力和未解决的挑战。

Conclusion: 为了实现材料信息学在多尺度工作流程中的更广泛整合，未来的关键在于解决数据标准、不确定性量化、可解释性和跨尺度一致性等核心障碍。论文明确了当前技术的可靠范围，并为未来的研究指明了方向。
...
### [42] [Polarization Engineering of the Orbital Hall Conductivity in Two-dimensional Ferroelectric Higher-Order Topological Insulator Tl$_2$S and SnS](https://arxiv.org/abs/2604.18093)
*YingJie Hu,Heng Gao,Yabei Wu,Wei Ren*

Main category: cond-mat.mtrl-sci

TL;DR: 该论文研究了二维铁电高阶拓扑绝缘体，发现其铁电极化可用于调控材料的拓扑性质和轨道霍尔电导率，为开发可控的轨道电子学器件提供了新途径。


<details>
  <summary>Details</summary>
Motivation: 铁电高阶拓扑绝缘体（HOTIs）因其铁电极化与能带拓扑之间的相互作用而展现出可调的物理特性。本研究旨在探索并利用这种耦合关系，以实现对材料轨道输运性质（特别是轨道霍尔效应）的有效控制。

Method: 通过对具有面外和面内两种不同极化方向的二维铁电HOTIs进行理论研究和模型分析，揭示了它们的拓扑起源和独特的轨道输运特性。研究以Tl2S和SnS分别作为两类HOTIs的代表性模型进行论证。

Result: 研究发现，面内极化与高阶拓扑之间存在强耦合，可以作为一种內禀的手段来可逆地切换能带隙内的轨道霍尔电导率平台。具体来说，在Tl2S模型中观察到持久的轨道输运，而在SnS模型中则实现了电可切换的轨道输运。

Conclusion: 该研究增进了我们对铁电性、高阶拓扑和轨道输运三者之间耦合关系的理解，为实现可控的轨道电子学开辟了新的道路。
...
### [43] [Evaluating dispersion models for ab initio simulation of G-I and G-II molten fluoride salts](https://arxiv.org/abs/2604.18139)
...
### [44] [Uncertainty-aware phase fraction prediction and active-learning-guided out-of-domain discovery of refractory multi-principal element alloys](https://arxiv.org/abs/2604.18322)
*A. K. Shargh,C. D. Stiles,J. A. El-Awady*

Main category: cond-mat.mtrl-sci

TL;DR: 本文提出了一种基于深度学习的框架，用于预测难熔多主元合金（RMPEAs）的相分数，并量化预测中的不确定性，从而通过主动学习策略加速发现新材料。


<details>
  <summary>Details</summary>
Motivation: 现有的机器学习方法在预测难熔多主元合金（RMPEAs）的相稳定性时，大多是确定性的，忽略了预测中固有的不确定性，这对于合金的稳健设计至关重要。

Method: 作者开发了一个基于混合密度网络（MDN）的深度学习框架来预测难熔多主元合金（RMPEAs）中的相分数，并量化偶然不确定性。他们使用CALPHAD衍生数据为多达六个组元相分别训练模型。此外，通过基于扰动的特征重要性分析来解决认知不确定性，并最终提出了一种基于不确定性的主动学习策略来发现新合金。

Result: 该方法在预测相分数方面实现了高准确性，并成功捕捉了相形成的概率性。研究还确定了一组最小且充分的输入特征，在保持预测性能和不确定性校准方面表现良好。最终，主动学习策略能够有效地指导发现含有新元素的、具有目标相的新型合金。

Conclusion: 该研究所提出的不确定性感知框架，有潜力加速和提高发现新型高性能合金的可靠性，并具有广泛的适用性。
...
### [45] [Direct observation of quadruple spin-texture locking in a 2D d-wave altermagnet](https://arxiv.org/abs/2604.18337)
...
TL;DR: 该研究利用自旋极化扫描隧道显微镜，首次在原子尺度上直接观测到反铁磁体候选材料RbV2Se2O中的自旋晶格锁定，证实了其d波反铁磁性，并发现了一种新的自旋条纹锁定现象。


<details>
  <summary>Details</summary>
Motivation: 反铁磁体（Altermagnets）是自旋电子学的新范式，但其关键特征“自旋晶格锁定”在原子尺度上的直接证据一直缺失。本研究旨在提供这一决定性的实验证据。

Method: 研究人员使用了一种原位、磁场可切换的自旋极化铬（Cr）针尖的扫描隧道显微镜（STM）。这使得他们能够在相同能量下绘制准粒子干涉的自旋对比图，从而克服了在该领域长期存在的实验障碍。

Result: 研究首次在原子尺度上提供了RbV2Se2O中d波自旋晶格锁定的证据。干涉图谱显示出显著的自旋相关调制，证实了实空间中的自旋散射锁定和倒易空间中的自旋动量锁定。此外，他们意外地发现了一种由长周期条纹调制引起的新现象——“自旋条纹锁定”，并将其归因于自旋密度波莫尔图案的出现。

Conclusion: 该研究为d波反铁磁体中的四重自旋织构锁定现象建立了统一的图像，证明了反铁磁体是探索自旋、晶格、动量、莫尔势和谷等多种自由度之间相互作用的通用平台。
...
### [46] [Plasmonic Photocatalysis Enables Selective Oxidative Coupling of Methane with Nitrous Oxide under Ambient Conditions](https://arxiv.org/abs/2604.18417)
...
### [47] [Moire Control of Alterelectric Quadrupolar Order](https://arxiv.org/abs/2604.18428)
...
### [48] [Signature of Unconventional Superconductivity in the High Temperature Normal State Resistivity](https://arxiv.org/abs/2604.16433)
...
### [49] [Quantum Tunnelling and Room-Temperature Superconductivity of Hydride from Size Effects](https://arxiv.org/abs/2604.16460)
*Xiaozhi Hu*
...
### [50] [Ultrafast Magneto-Pressure Spectroscopy and Control of Correlated Phases in a Trilayer Nickelate](https://arxiv.org/abs/2604.16611)
...
### [51] [Pairing Mechanism in Bilayer Nickelate La$_3$Ni$_2$O$_7$ Superconductors](https://arxiv.org/abs/2604.17181)
...
### [52] [Collective Resonance of Superconducting/Normal Domain Walls in the Intermediate State of type-I superconductor](https://arxiv.org/abs/2604.17333)
...
### [53] [Anisotropic Superconducting Diode Effect in Planar Josephson Junctions](https://arxiv.org/abs/2604.17594)
...
### [54] [Metal Atom (Dis)Order and Superconductivity in YCaH$_{n}$ ($n=8-20$) High-Pressure Superhydrides](https://arxiv.org/abs/2604.17712)
*Masashi W. Kimura,Seong Won Jang,Nisha Geng,Eva Zurek*

Main category: cond-mat.supr-con

TL;DR: 本文使用密度泛函理论（DFT）计算研究了Y-Ca三元超氢化物，发现金属原子无序对稳定性和超导性有重要影响，通过掺杂调控可以显著提高或降低其超导临界温度。


<details>
  <summary>Details</summary>
Motivation: 高压超氢化物因其高超导临界温度（Tc）而备受关注。本研究旨在通过理论计算，探索Y-Ca三元超氢化物的结构、性质，特别是金属原子无序的可能性以及掺杂对Tc的提升效果。

Method: 该研究采用密度泛函理论（DFT）进行计算。具体来说，他们使用各向同性Eliashberg形式主义来计算超导临界温度Tc。

Result: 研究发现：1) 对于YCaH8，多种金属原子排布的相能量相近，表明构型熵很重要，其Tc在180 GPa下可达149-170 K。2) YCaH12同样存在无序，不同有序变体的Tc范围很广（105-253 K）。3) 对于YCaH18和YCaH20，只预测到一种稳定的结构。

Conclusion: Y-Ca三元超氢化物的稳定性和超导性受组分和原子排布的复杂影响。金属原子的无序在某些组分中是重要特征。通过混合金属原子（掺杂）来调节费米能级可以有效调控超导临界温度，但其效果（增强或减弱）取决于具体的氢化物结构。
...
### [55] [Pairing properties of correlated three-leg ladders with strong interchain couplings near 1/3 filling](https://arxiv.org/abs/2604.17812)
*Yushi Yamada,Tatsuya Kaneko,Masataka Kakoi,Ryota Ueda,Kazuhiko Kuroki*

Main category: cond-mat.supr-con

TL;DR: 本文使用密度矩阵重整化群方法研究了三腿梯子模型（t-J 和 Hubbard 模型）。研究发现，在1/3填充状态下掺杂空穴会产生超导配对，而掺杂电子则不会。这一结果为理解三层镍基超导体的电子特性提供了见解。


<details>
  <summary>Details</summary>
Motivation: 本研究的动机是为了理解三层结构镍基超导体（trilayer nickelate superconductors）的电子特性，特别是研究在1/3填充附近的三腿梯子模型的基态性质。

Method: 作者应用了密度矩阵重整化群（DMRG）方法来研究三腿 t-J 梯子模型和三腿 Hubbard 模型。

Result: 在1/3填充的自旋能隙态中掺杂空穴时，研究发现配对关联函数呈现幂律衰减（暗示超导），而自旋关联则呈指数衰减。相比之下，掺杂电子则不会产生显著的配对关联。

Conclusion: 数值计算结果表明，在三腿梯子模型中，空穴掺杂可以诱导出超导配对状态，而电子掺杂则不能。这些发现为理解三层镍基超导体的电子特性提供了理论见解。
...
### [56] [$0-π$ transitions in non-Hermitian magnetic Josephson junctions](https://arxiv.org/abs/2604.17978)
...
### [57] [Superconductivity in Ruddlesden-Popper nickelates: a review of recent progress, focusing on thin films](https://arxiv.org/abs/2604.18385)
...
### [58] [New Physics Reach through Precision at Future Colliders: a Multi-Pronged Approach](https://arxiv.org/abs/2604.16596)
...
### [59] [Four-fermion operators, $Z$-boson exchange, and $τ$ lepton dipole moments](https://arxiv.org/abs/2604.16598)
...
### [60] [The effect of the two-loop SMEFT RGEs at future colliders](https://arxiv.org/abs/2604.16599)
...
### [61] [Baryon Asymmetry from Electroweak-Symmetric Domain Walls](https://arxiv.org/abs/2604.16603)
*Jacopo Azzola,Oleksii Matsedonskyi,Andreas Weiler*

Main category: hep-ph

TL;DR: 本文研究了由畴壁（domain walls）驱动的电弱重子生成机制，发现重子产额由畴壁宽度等多个尺度共同决定，并确定了在特定模型下能够解释观测事实的参数空间。


<details>
  <summary>Details</summary>
Motivation: 为了解释宇宙中观测到的物质-反物质不对称（即重子不对称性）的来源，本文探索了一种新的电弱重子生成机制，该机制依赖于具有电弱对称核心的畴壁（Domain Walls）。

Method: 作者在厚壁机制下，通过分析CP破坏力如何产生手性不对称性，并结合输运过程与弱sphaleron过程来计算重子产额。他们构建了一个简化的模型来捕捉畴壁两侧的干涉效应，并将其应用于一个单重态扩展的标准模型中。

Result: 研究发现，重子产额由畴壁宽度、CP破坏源宽度和扩散长度这三者之间的层级关系决定。一个显著的发现是畴壁两侧会发生干涉，导致重子生成过程对CP破坏源的对称性表现出不同的行为。所构建的简化模型在很宽的参数范围内与完整的输运系统预测一致。

Conclusion: 该研究证实，通过电弱对称畴壁在等离子体中移动是一种可行的重子生成机制，并成功描绘了在单重态扩展标准模型中能够产生观测到的重子不对称性的参数区域。
...
### [62] [Investigating the Neutrino Mass Ordering Problem via Ternary Plots](https://arxiv.org/abs/2604.16652)
...
### [63] [Probing Flavor-Violating Higgs Decays in the Type-III Two-Higgs-Doublet Model at the LHC and HL-LHC](https://arxiv.org/abs/2604.16679)
...
### [64] [Four-loop QCD mixing of current-current operators](https://arxiv.org/abs/2604.16691)
...
### [65] [Cavity-mode couplings in axion dark matter searches](https://arxiv.org/abs/2604.16891)
*Byeong Rok Ko*
...
### [66] [Proposed mixing between $2P$ and $1F$ wave charmonia](https://arxiv.org/abs/2604.17193)
...
### [67] [Deciphering the universal scaling of particle transverse momentum spectra in heavy-ion collisions](https://arxiv.org/abs/2604.17345)
...
### [68] [Probes for CP Violation in B Decays at the FCC: A Theorist's Perspective](https://arxiv.org/abs/2604.17372)
*Robert Fleischer*
...
### [69] [Hydrodynamics of Filtered Dark Matter: A Two-Component Approach](https://arxiv.org/abs/2604.17383)
*Juntaro Wada*
...
### [70] [Boost-invariant perfect Fermi-Dirac spin hydrodynamics](https://arxiv.org/abs/2604.17392)
...
### [71] [Entropy and mean multiplicity from dipole models in the high energy limit](https://arxiv.org/abs/2604.17418)
...
### [72] [Testing $α$-attractor P-model of inflation by Cosmic Microwave Background radiation](https://arxiv.org/abs/2604.17430)
...
### [73] [A Unified Bogoliubov Approach to Primordial Gravitational Waves: From Inflation to Reheating](https://arxiv.org/abs/2604.17478)
...
### [74] [Shedding New Light on the ${B \to πK}$ Puzzle](https://arxiv.org/abs/2604.17496)
*Eleftheria Malami*
...
### [75] ["Neutrinoless double beta decay" is the correct name for neutrinoless double beta decay](https://arxiv.org/abs/2604.17545)
*James M. Cline*
...
### [76] [Sequential Y(nS) suppression in high-multiplicity pp collisions: the experimental case for an early, globally correlated medium](https://arxiv.org/abs/2604.17657)
*Renato Campanini*
...
### [77] [New physics in the $ZZh$ vertex: One-loop contributions from a radiative seesaw model](https://arxiv.org/abs/2604.17666)
...
### [78] [Searching for dark photons in $J/ψ$ decays](https://arxiv.org/abs/2604.17726)
...
### [79] [Polarization, Maximal Concurrence, and Pure States in High-Energy Collisions](https://arxiv.org/abs/2604.17756)
*Yu-Xuan Liu,Luo-Ting He,Bo-Wen Xiao*

Main category: hep-ph

TL;DR: 这篇论文发现，在一个量子系统中，局部自旋的极化程度越高，系统能达到的最大量子纠缠就越低。


<details>
  <summary>Details</summary>
Motivation: 本文旨在建立双量子比特系统（two-qubit systems）中局部自旋极化与量子纠缠之间的定量关系。

Method: 研究人员通过在固定的局部极化条件下，推导并发度（concurrence）的上限来建立两者关系。随后，他们将这个框架应用于一个具体的物理过程（$e^+e^- \to Z^0 \to q\bar{q}$）进行验证。

Result: 研究发现，增加局部极化会限制可实现的最大纠缠。在具体的物理应用中，最大纠缠出现在特定的运动学区域，并且相对于无极化的情况显著减少。

Conclusion: 该论文建立了一个通用的、与具体过程无关的框架，将局部极化、最大纠缠和纯态的角色联系起来。
...
### [80] [Soft mode dynamics associated with QCD critical point and color superconductivity -- pseudogap, anomalous dilepton production and electric conductivity](https://arxiv.org/abs/2604.17764)
...
### [81] [Three-body decay $φ\toπ^+π^-π^0$ with Omnès-type final-state interactions](https://arxiv.org/abs/2604.17780)
*Seung-il Nam,Jung Keun Ahn*

Main category: hep-ph

TL;DR: 该论文研究了 φ→3π 衰变过程，使用一个有效拉格朗日框架，该框架分离了主导的 ρπ 机制和直接的三π项。研究发现，ππ 的末态相互作用效应是显著的，虽然理论计算结果与实验值接近，但在相空间边界仍存在差异，表明需要更精确的模型。


<details>
  <summary>Details</summary>
Motivation: 本研究的动机是在一个清晰的唯象学框架下，探究 φ→π+π-π0 衰变中，主导的 ρπ 机制与直接三π产生机制之间的关系，并分离出并量化 ππ 末态相互作用（重散射）所带来的主要影响。

Method: 该研究采用了一种有效拉格朗日框架。在振幅层面，模型明确分开了主导的 ρπ 机制和直接的三π项。共振贡献由 Gounaris-Sakurai 传播子描述，中性道包含了 ρ0-ω 混合效应。关键的 P波 ππ 末态相互作用通过一个恒定的在壳 Omnès 因子 C_Ω 来引入。

Result: 计算得到的衰变宽度 Γ_th=0.6950 MeV，比实验值高出约5%。直接衰变项的积分权重与实际水平相符。计算出的在壳 Omnès 因子值为 C_Ω=4.794，表明 ππ 重散射效应提供了显著的增强。尽管如此，在达利兹图的相空间边界附近，理论预测与实验数据仍存在明显差异。

Conclusion: 当前的模型是一个中间的唯象学步骤，而非精确的振幅分析。虽然该方法成功分离并量化了 ππ 重散射的关键作用，但剩余的差异表明需要更精细的处理。未来的工作方向是引入依赖于能量的 Omnès 因子，并直接对 KLOE 实验的达利兹图数据进行拟合。
...
### [82] [Compositeness of near-threshold eigenstates with Coulomb plus short-range interactions](https://arxiv.org/abs/2604.17813)
*Tomona Kinugawa,Tetsuo Hyodo*

Main category: hep-ph

TL;DR: 本文采用有效场论研究了库仑相互作用如何影响近阈值二体系统的内部结构（复合度），发现强库仑力会抑制束缚态的复合成分，这与纯短程力系统的情况相反。


<details>
  <summary>Details</summary>
Motivation: 理解弱束缚系统（如一些奇特强子和原子核）的内部结构是一个重要课题。这些系统通常同时存在短程的核力与长程的库仑力，而已有的纯短程力理论可能不适用。因此本研究旨在建立一个新框架，用以分析在库仑相互作用存在时系统的“复合度”（即一个状态在多大程度上是其组分粒子的束缚态）。

Method: 作者们使用了非相对论有效场论，推导出了一个适用于包含库仑相互作用系统的“复合度”表达式。该公式将复合度与库仑散射长度、有效程等散射参数联系起来。随后，他们通过数值研究，分析了极点轨迹和复合度如何随库仑相互作用的强度变化。

Result: 研究发现，库仑相互作用显著改变了系统在束缚阈值附近的行为。当库仑力较强时，近阈值态的复合度不会像纯短程力系统那样显著增大。而在库仑力较弱时，系统仍保留了部分短程普适性的特征。此外，即使是共振态，在库仑力存在时也主要由复合成分主导。

Conclusion: 论文的结论是，库仑相互作用从根本上改变了近阈值量子态的内部结构。源于纯短程力系统的传统认知（即弱束缚态是高度复合的）不再普适。该研究建立的理论为分析同时包含两种相互作用的真实物理系统（如奇特强子和原子核）的结构提供了新工具。
...
### [83] [Probing Cosmic-Ray-Boosted and Supernova-Sourced Sub-GeV Dark Matter with Paleo-Detectors](https://arxiv.org/abs/2604.17869)
...
### [84] [Large CP violation in $Λ_b\rightarrow ΛD$ decays and extraction of the Cabibbo-Kobayashi-Maskawa angle $γ$](https://arxiv.org/abs/2604.17877)
*Zhou Rui,Zhi-Tian Zou,Ya Li,Ying Li*

Main category: hep-ph

TL;DR: 该论文指出，$Λ_b \to ΛD$ 衰变过程存在巨大的 CP 破坏效应，其不对称性可达 50%，并提出了一种利用此衰变过程来精确测量 CKM 角 γ 的新方法，为 LHCb 实验提供了重要目标。


<details>
  <summary>Details</summary>
Motivation: 首次在 b 重子衰变中观测到 CP 破坏现象，激发了学界去寻找其他具有大的 CP 破坏效应的重子衰变过程。

Method: 作者提出研究 $Λ_b \to ΛD$ 衰变过程，并通过计算 CP 不对称性、与角分布相关的 CP 破坏可观测量，以及结合这些数据来提取 CKM 角 γ。

Result: 1. 预测在 $Λ_b \to ΛD$ 衰变中，CP 不对称性的幅度可高达 50%。
2. 首次预测了几个与角分布参数相关的非零 CP 破坏可观测量。
3. 提出了一种在重子衰变中提取 CKM 角 γ 的新策略。

Conclusion: $Λ_b \to ΛD$ 衰变是在重子领域中寻找 CP 破坏和确定 CKM 角 γ 最有希望的候选者之一，该研究为未来的理论和实验研究提供了新的方向。
...
### [85] [Jet Quenching in the Smallest Hadronic Collision Systems](https://arxiv.org/abs/2604.18010)
*Coleridge Faraday,Ben Bert,Jack Brand,Werner Vogelsang,W. A. Horowitz*

Main category: hep-ph

TL;DR: 本文使用pQCD预测了轻离子碰撞中的粒子产额，发现在极小系统中也存在能量损失，并指出观测到的v2并非源于此能量损失机制。


<details>
  <summary>Details</summary>
Motivation: 研究在极轻的离子碰撞系统中是否存在夸克-胶子等离子体（QGP）的形成以及伴随的能量损失效应。同时，探究在小型碰撞系统（如 p+Pb）中观测到的集体行为（大 v2）的来源。

Method: 该研究采用了微扰量子色动力学（pQCD）的理论框架，对多种轻离子碰撞系统（如³He+³He, ⁶Li+⁶Li等）中的高动量粒子产额进行了理论预测，并对比了包含和不包含介质能量损失两种情况下的结果。

Result: 研究发现，即使在极小的³He+³He系统中也存在不可忽略的粒子产额压低现象。³He和⁶Li碰撞系统为观测部分子能量损失提供了洁净的环境。此外，模型计算显示，能量损失机制本身在小系统中产生的椭圆流（v2）几乎为零。

Conclusion: 论文断定，在小型碰撞系统（如p+Pb）中观测到的大椭圆流（v2）并非由末态部分子的能量损失所导致。这表明夸克-胶子等离子体（QGP）的特征（能量损失）可以在极小系统中出现，但其集体行为的来源需要其他解释。
...
### [86] [Precision calculations for electroweak multi-boson processes](https://arxiv.org/abs/2604.18025)
*S. Dittmaier*

Main category: hep-ph

TL;DR: 这篇论文回顾了大型强子对撞机中，两个和三个弱规范玻色子产生的次领先阶 (NLO) QCD 和电弱修正。研究发现电弱修正效应非常显著，并讨论了不同近似计算方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 为了精确预测大型强子对撞机 (LHC) 上弱规范玻色子的散射与产生过程，需要计算高阶量子修正。准确理解和计算这些修正（特别是电弱修正）对于精确的理论预测至关重要。

Method: 作者回顾并展示了针对同符号WW散射和三W玻色子产生等典型过程的次领先阶 (NLO) QCD 和电弱修正的计算结果。同时，他们将完整的离壳计算结果与“矢量玻色子散射近似”、“主导极点近似”和“有效矢量玻色子近似”等多种近似方法的计算结果进行了比较。

Result: 研究发现，纯电弱修正的影响非常显著。即使对于积分截面，这些修正对于同符号WW散射过程和三W玻色子产生过程，通常会分别达到约-16%和约-7%的量级。

Conclusion: 在精确计算弱规范玻色子的相关过程中，次领先阶的电弱修正是一个不可忽略的重要因素。此外，论文还评估了不同近似计算方法的适用性，为未来类似研究提供了参考。
...
### [87] [A Type-I Seesaw Framework with Non-Holomorphic Modular Symmetry](https://arxiv.org/abs/2604.18070)
*Cheshta,Priya,Suneel Dutt,B. C. Chauhan*

Main category: hep-ph

TL;DR: 该论文研究了一种非完整模对称模型来解释中微子质量，发现该模型与正序（NH）数据吻合良好，但排除了反序（IH）。


<details>
  <summary>Details</summary>
Motivation: 在由屈和丁提出的非完整模对称（non-holomorphic modular symmetry）框架内，研究中微子质量的生成机制及其可行性。

Method: 在非完整模对称框架下，通过第一类跷跷板机制（Type-I seesaw）生成中微子质量，并使用当前的中微子振荡数据进行$χ^2$（卡方）分析来检验模型的有效性。

Result: 对于正序（NH），模型的拟合结果良好（最小$χ^2$为7.06），大部分参数在$1σ$范围内，但预测大气混合角$\sin^2θ_{23}$在第二卦限。对于反序（IH），模型与实验数据严重不符（$χ^2$ > 100），因此被排除。

Conclusion: 该模型在正序（NH）情况下是可行的，并做出了一些可被未来长基线中微子实验检验的预测，例如大气混合角和CP破坏相位的范围。同时，该模型排除了反序（IH）的可能性。
...
### [88] [Sexaquarks and $H$ dibaryons in the $uuddss$ system: a comparison within a constituent quark model](https://arxiv.org/abs/2604.18174)
*M. C. Gordillo*
...
### [89] [Analysis of the $D_0^*(2300)$ resonance from lattice QCD under chiral symmetry](https://arxiv.org/abs/2604.18286)
...
### [90] [Generalized parton distributions of valence, sea, and gluon components of the proton](https://arxiv.org/abs/2604.18355)
...
### [91] [Two-body charmed anti-charmed baryonic $B$ decays](https://arxiv.org/abs/2604.18366)
*Chun-Khiang Chua*
...
### [92] [Looking for Lights from the Darkness: Signals from MeV-scale Solar Axion-like Particles](https://arxiv.org/abs/2604.18397)
...
### [93] [New Physics in inclusive $\bar{B} \to X_c \ell \barν$ decays](https://arxiv.org/abs/2604.18443)
...
### [94] [Three loop QCD corrections to electroweak radiative parameters](https://arxiv.org/abs/2604.18516)
...
### [95] [Modelling Gas-Phase Reaction Kinetics with Guided Particle Diffusion Sampling](https://arxiv.org/abs/2604.16461)
...
### [96] [Coarse-Grained Dynamics with Spatial Disorder and Non-Markovian Memory](https://arxiv.org/abs/2604.16825)
...
### [97] [Consistent control of energy dissipation in non-spherical particle contact via a structure-preserving formulation](https://arxiv.org/abs/2604.18494)
*Y. T. Feng*
...
### [98] [Parametric Resonance and RF-to-THz Frequency Conversion in Semiconductor Plasmonic Crystals](https://arxiv.org/abs/2604.16510)
...
### [106] [Microscopic Theory of Acoustic Phonon Scattering by Charge-Density-Wave Fluctuations](https://arxiv.org/abs/2604.17655)
*Han Huang*
...
### [107] [Polarized light Raman scattering by an atom near an ultrathin periodically aligned carbon nanotube film](https://arxiv.org/abs/2604.17246)
...
### [108] [Quantum higher-spin Hall insulators](https://arxiv.org/abs/2604.17479)
...
### [109] [Photocurrent at oblique illumination and reconstruction of wavefront direction with 2d photodetectors](https://arxiv.org/abs/2604.17591)
...
### [110] [Crystallography, Lorentz violation, and the Standard-Model Extension](https://arxiv.org/abs/2604.17646)
...
### [111] [Magnetoresistance from decoherence](https://arxiv.org/abs/2604.17672)
...
### [112] [Dynamical spin-nematic order in a transverse field Ising chain with non-Hermitian Gamma interaction](https://arxiv.org/abs/2604.17700)
...
### [113] [Propagation, generation, and utilization of topologically trivial magnetic solitons in magnetic nanowires](https://arxiv.org/abs/2604.17992)
...
### [114] [Generic skyrmion phase diagram in ferrimagnetic films](https://arxiv.org/abs/2604.18079)
...
### [115] [Asymmetric Scattering-Induced Neel Spin-Orbit Torque in Antiferromagnets](https://arxiv.org/abs/2604.18097)
*Sayan Sarkar,Amit Agarwal*

Main category: cond-mat.mes-hall

TL;DR: 本文提出了一种新的反铁磁体翻转机制，通过非对称杂质散射和电子能带几何的相互作用来产生有效的尼尔自旋轨道力矩（NSOT）。


<details>
  <summary>Details</summary>
Motivation: 在共线反铁磁体中，传统的尼尔自旋轨道力矩（NSOT）依赖于对称散射过程，研究者旨在探索是否存在其他机制。

Method: 作者提出了一种理论机制，即非对称杂质散射与布洛赫电子的反常自旋极化率（ASP）相结合。他们使用四方晶系CuMnAs的简化模型来计算这种“反常斜散射”的贡献。

Result: 研究表明，这种源于能带几何和非对称散射相互作用的新机制，可以产生与传统机制相当甚至更强的NSOT贡献，尤其是在高杂质密度下。

Conclusion: 该研究确定了一种由能带几何驱动的全新NSOT产生机制，为高效电学控制反铁磁材料提供了新的途径。
